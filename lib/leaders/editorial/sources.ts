...
export async function fetchCatalogSource(id:string){
 ...
 try{response=await fetch(source.url,{redirect:'error',signal:AbortSignal.timeout(15000),headers:{Accept:'text/html,text/plain','User-Agent':'VisionSeek-Editorial/1.0'},cache:'no-store'});}
 catch{throw new RoomError(502,'SOURCE_UNAVAILABLE');}
