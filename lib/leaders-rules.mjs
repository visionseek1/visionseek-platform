 ...
 * A real field gets one of its crops. A post with no field gets no image.
 * If that crop matches a recent card, the next crop in the pool is used.
 */
export function orderedCoverRows(items, extras = []) {
  const rows = [];
  ...
  for (const extra of [...extraRows(), ...extras]) {
    if (!extra?.slug || !extra?.publishedAt) continue;
